        ...
        try:
            ...
            if model_name == SelfHostedModels.MODEL_DATABRICKS_DBRX:
                prompt = [
                    {
                        "role": "system",
                        "content": system_prompt.replace("[context]", "").replace("[question]", "").replace("[hint]", "").replace("[examples]", ""),
                    },
            ...
            sampling_params = SamplingParams(max_tokens=Defaults.MAX_TOKENS_TO_GENERATE, use_cache=True)
            output = model.generate(inputs, sampling_params)
